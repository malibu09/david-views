// Controles de reproducción para Hero y portafolio.
const allVideos=[...document.querySelectorAll('video')];
function syncVideoButton(video){const btn=video.parentElement?.querySelector('.video-toggle');if(!btn)return;const playing=!video.paused&&!video.ended;btn.textContent=playing?'Ⅱ':'▶';btn.setAttribute('aria-label',playing?'Pausar video':'Reproducir video');}
function pauseOtherVideos(active){allVideos.forEach(v=>{if(v!==active&&!v.paused)v.pause();});}
allVideos.forEach(video=>{const btn=video.parentElement?.querySelector('.video-toggle');if(btn)btn.addEventListener('click',()=>{if(video.paused){pauseOtherVideos(video);video.play().catch(()=>{});}else video.pause();});['play','pause','ended','loadeddata'].forEach(evt=>video.addEventListener(evt,()=>syncVideoButton(video)));syncVideoButton(video);});
const heroVideo=document.querySelector('.hero-video');if(heroVideo){heroVideo.muted=true;heroVideo.defaultMuted=true;heroVideo.playsInline=true;const ready=()=>heroVideo.classList.add('is-ready');heroVideo.addEventListener('loadeddata',ready,{once:true});heroVideo.addEventListener('canplay',()=>{ready();heroVideo.play().catch(()=>syncVideoButton(heroVideo));},{once:true});if(heroVideo.readyState>=2){ready();heroVideo.play().catch(()=>syncVideoButton(heroVideo));}else heroVideo.load();}
